...
function prepLoadedMusic(newState: ScoreState, action: ScoreAction): ScoreState {
	console.log('prepLoadedMusic entry')
    newState.image = `music/${action.filename}/${action.filename}.svg`,
    newState.scoreData = action.scoreData
...
function calcPixels(metrics: ScoreMetrics, desiredHeight: number):ScoreMetrics {
    ...
    const newPx = {
        ...
        staves: new Array<StaffMetric>(metrics.staves.length),
    }
